...
def find_starting_bound(test_finish, test_stocks):
    ...
    bound_finish = [round((stock_weight/stock_width * v['width'] + v['need_cut'])/v['average FC']) for _, v in test_finish.items()]
    # Check if any number in the list is >= 2
    if any(num >= 2 for num in bound_finish):
        starting_bound = 2
    else:
        starting_bound = 1
...
def multistocks_cut(logger, finish, stocks, MATERIALPROPS, margin_df, prob_type):
    ...
    while cond == True: # have stocks and need cut
        ...
        # print(f"length solution {len_last_sol}")
    ...
    try:  ## Add try-catch for any error happen -> go to next job
        ...
        if not finish: # EMPTY
            ...
            while coil_center_priority_cond:
                ...
                # FIND STARTING BOUND
                starting_bound = find_starting_bound(finish, partial_stocks)
