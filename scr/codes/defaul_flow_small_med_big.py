...
def partial_stock(stocks, allowed_cut_weight, group, medium_mc_weight):
    ...
    if "big" in group:
        ...
        filtered_stocks = dict(sorted(filtered_stocks.items(), key=lambda x: (x[1]['weight'], x[1]['receiving_date'])))
    elif (group in ["medium","medium1","medium2",'medium3'] and medium_mc_weight*2 > max_mc_wg):
        # Sort weight ascending and receiving_date ascending
        filtered_stocks = dict(sorted(stocks.items(), key=lambda x: (x[1]['weight'], x[1]['receiving_date'])))
    elif (group in ["medium","medium1","medium2",'medium3'] and medium_mc_weight*2 <= max_mc_wg):
        # Sort weight descending and receiving_date ascending
        filtered_stocks = dict(sorted(stocks.items(), key=lambda x: (x[1]['weight'], -x[1]['receiving_date']), reverse= True))
    else: 
        # Sort weight descending and receiving_date ascending
        filtered_stocks = dict(sorted(stocks.items(), key=lambda x: (x[1]['weight'], -x[1]['receiving_date']), reverse= True))
...
def partial_finish(finish, stock_ratio):
    ...
    res = dict(sorted(partial_finish.items(), key=lambda x: (x[1]['need_cut']))) # need_cut van dang am sort ascending               
    return res
...
def average_3m_fc(finish):
    average_3fc = {f: round(f_info['average FC'])+1 for f, f_info in finish.items()} # plus 1 for zero forecast
    return average_3fc
...
def transform_to_df(data):
    ...
    flattened_data = []
    for item in data:
        common_data = {k: v for k, v in item.items() if k not in ['count','cuts','cut_width' ,"cut_wg", "remark"]}
        for cut, line in item['cuts'].items():
            if line > 0:
                flattened_item = {**common_data, 'cuts': cut, 
                                  'lines': line,
                                  'cut_width': item['cut_width'][cut],
                                  'cut_weight': item['cut_wg'][cut],
...
def find_optimal_bound_step(test_finish, test_stocks, MATERIALPROPS, margin_df):
    ...
    if len(test_finish) > 7:
        bound_range = [0.5, 0.75, 1.0]
    else:
...
def multistocks_cut(logger, finish, stocks, MATERIALPROPS, margin_df, prob_type):
    ...
    while cond == True: # have stocks and need cut
        ...
        if ins_bound and bound == max_bound: # empty solution
            ...
            negative_over_cut_ratio = sum(value < -0.03 for value in over_cut_rate.values()) > 0
    ...
    for finish_item in finish_list['materialprop_finish'][materialprop_set]['group']:
        try:  ## Add try-catch for any error happen -> go to next job
            ...
            while j < no_warehouse: 
                ...
                partial_stocks = partial_stock(filtered_stocks_by_wh, total_need_cut_by_cust_gr * mc_ratio, group_name, medium_mc_weight)
            ...
            while coil_center_priority_cond: ### OPERATOR COIL CENTER to cut until all FG DONE P3.0
                ...
                if coil_center_priority_cond:
                    ...
                    partial_stocks = partial_stock(next_warehouse_stocks, total_over_cut * mc_ratio, group_name, medium_mc_weight)
