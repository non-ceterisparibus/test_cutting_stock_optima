...
def filter_single_stock_df_to_dict(df, stock_id, value_columns, sum_of_total_need_cuts):
    ...
    sorted_df = choose_stocks_by_need_cuts(sorted_df, sum_of_total_need_cuts, over_cut_rate = 0.5)
...
def create_upper_bound_need_cut(df, BOUND_VALUE=None):
    ...
    if BOUND_VALUE is not None:
        ...
        df['upper_bound'] = np.where(df['need_cut'] < 0, 0.3 * df['fc1'] - df['need_cut'], np.nan)
        df['upper_bound_po']  = df['upper_bound'] 
