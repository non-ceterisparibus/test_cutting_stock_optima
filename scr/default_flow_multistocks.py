...
def average_3m_fc(finish):
    average_3fc = {
        key: (
            sum(v for v in (finish[key]['fc1'], finish[key]['fc2'], finish[key]['fc3']) 
                if isinstance(v, (int, float)) and not math.isnan(v)) /
            sum(1 for v in (finish[key]['fc1'], finish[key]['fc2'], finish[key]['fc3']) 
                if isinstance(v, (int, float)) and not math.isnan(v))
        ) if any(isinstance(v, (int, float)) and not math.isnan(v) 
                for v in (finish[key]['fc1'], finish[key]['fc2'], finish[key]['fc3'])) 
        else float('nan')
...
logging.basicConfig(filename=f'scr/log/multi-flow-{formatted_date}.log', level=logging.INFO, 
