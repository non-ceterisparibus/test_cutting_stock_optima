...
def multistocks_cut(logger, finish, stocks, PARAMS, bound, margin_df, prob_type):
    "FUNCTION to cut all possible stock with finish demand upto upperbound"
...
logging.basicConfig(filename=f'scr/log/cutting-stocks_{formatted_date}.log', level=logging.INFO, 
            ...
            while coil_center_priority_cond: ### OPERATOR COIL CENTER to cut until all FG-in MIN MAX GR DONE
                ...
                while bound < 4: ### ### OPERATOR UPPER BOUND = {1, 2, 3}
                    ...
                    try:
                        ...
                        #### SEMI CASE
                        if len(stocks_by_wh) == 1 and len(finish) == 1 and bound == 1: 
                        ...
                        else: pass ### NO SOLUTION for REWIND va SEMI
                    ...
                    if not final_solution_patterns:
                        ...
                        filename_excel = f"scr/results/solution-{cleaned_param_set}-{customer_name}-{warehouse_order[j]}.xlsx"
                        df.to_excel(filename_excel, index=False)
                        logger.info(f"->>>> The solution {param_set}-{customer_name}- bound {bound} EXCEL saved")

                        # --- to JSON ---
                        # filename_json = f"scr/results/solution-{cleaned_param_set}-{customer_name}-{warehouse_order[i]}.json"
                        # save_to_json(filename_json, final_solution_patterns)
                        # logger.info(f"->>>> The solution {param_set}-{customer_name}- bound {bound} JSON saved")
                        
                        break 
                    
                # To next COIL CENTER priority if still able
                if j < 2: 
                    j +=1
